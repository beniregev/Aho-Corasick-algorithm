...
public class SetMatchQueue {
    ...
    public boolean matchAndClear(String haystack, SetMatchListener listener, int purgeToIndex) {
        ...
        if (!isEmpty()) {
            ...
            while (i < emptySlotIdx) {
                if (endIndexes[i] <= purgeToIndex) {
                    if (!listener.match(haystack, startIndexes[i], endIndexes[i])) {
                        return false;
                    }
                } else {
                    break;
